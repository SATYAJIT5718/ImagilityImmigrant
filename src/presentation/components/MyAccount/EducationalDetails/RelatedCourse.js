...
const RelatedCourse = ({
  ...
  const data = editData?.courses[0]?.courseName
    ? editData.courses[0].courseName.split(',').map((item, index) => {
  ...
  const addHanderler = () => {
    let previousData = editData?.courses[0]?.courseName
      ? editData.courses[0].courseName.split(',')
      : [];
    let data = [...previousData, ...tags.tagsArray].toString();
    setFieldValue('courseName', data),
      tags.tagsArray.length > 0
        ? (setTags({tagsArray: []}), handleSubmit())
        : Toast.show('Please Enter Courses Name', Toast.SHORT);
  };
  ...
  const deleteHandler = id => {
    const filterData = data
      ...
      .join(',');
    setFieldValue('courseName', filterData);
    handleSubmit();
  ...
  return (
    <View style={{marginTop: scale(10), marginBottom: scale(10)}}>
      <Text style={styles.formInputData}>List of Courses</Text>
      <Text style={styles.bodyText}>
        Enter all relevant courses taken as a part of your degree programme
        using a comma to separate.
      </Text>
      <View style={{flexDirection: 'column'}}>
        <Text style={styles.formInputTitle}>Courses Name</Text>
...
const mapStateToProps = ({sponsorDetailsReducer: {educationalInfo}}) => ({
  educationalInfo,
});
